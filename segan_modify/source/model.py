...
class SEGAN(Model):
    ...
    def __init__(self, sess, args, devices, features=None, wavbatch=None, infer=False, name='SEGAN'):
        ...
        if wavbatch!= None:
            self.build_model(args, features, wavbatch)

    def build_model(self, config, features=None, wavbatch=None):
        all_d_grads = []
        all_g_grads = []
...
#         with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
        ...
        idx = 0
        self.build_model_single_gpu(idx, features, wavbatch)
        ...
        with tf.variable_scope(tf.get_variable_scope(), reuse = tf.AUTO_REUSE):
            avg_d_grads = average_gradients(all_d_grads)
            avg_g_grads = average_gradients(all_g_grads)
            self.d_opt = d_opt.apply_gradients(avg_d_grads, global_step=tf.train.get_global_step())
            self.g_opt = g_opt.apply_gradients(avg_g_grads, global_step=tf.train.get_global_step())

    def build_model_single_gpu(self, gpu_idx, features=None, wavbatch=None):
        noisybatch = None
        if self.args.sagemaker == 'false':
            if gpu_idx == 0:
        ...
        if gpu_idx == 0:
            ...
            #                                  name='sample_wavs')
            ref_Gs = None
            if self.args.g_type == 'ae':
                ref_Gs = self.generator(noisybatch, is_ref=True,
                                    spk=None,
                                    do_prelu=do_prelu)
            elif self.args.g_type == 'dwave':
                ref_Gs = self.generator(noisybatch, is_ref=True,
                                    spk=None)
#             ref_Gs = self.generator(noisybatch, is_ref=True,
#                                     spk=None,
...
#         D_fk_joint = tf.concat(2, [G, noisybatch])
    ...
    def train(self, config, devices):
        ...
        if self.load(self.save_path, self.weights):
            print('[*] Load SUCCESS')
        else:
            print('[!] Load failed')
        batch_idx = 0
